  ...
  return (
    <Box sx={{ cursor: 'zoom-in', paddingTop: '100%', position: 'relative' }}>
      <LargeImgStyle
        ...
        onClick={() => onOpenLightbox(item.thumbnail)}
      />
    </Box>
  );
}
...
function ThumbnailItem({ item }) {
  return (
    <ThumbWrapperStyle>
      ...
      <ThumbImgStyle alt="thumb image" src={`data:image/*;base64,${item.thumbnail}`} />
    </ThumbWrapperStyle>
  );
}
...
export default function ProductDetailsCarousel() {
  ...
  const imagesLightbox = product.thumbnail.map((_image) => _image.thumbnail);
  ...
  const settings2 = {
    ...
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: product.thumbnail.length > 3 ? 3 : product.thumbnail.length
  };
    ...
    <RootStyle>
      <Box sx={{ p: 1 }}>
        ...
        >
          ...
          <CarouselControlsArrowsIndex
            index={currentIndex}
            total={product.thumbnail.length}
            onNext={handleNext}
      ...
      <Box
        sx={{
          ...
          ...(product.thumbnail.length === 1 && { maxWidth: THUMB_SIZE * 1 + 16 }),
          ...(product.thumbnail.length === 2 && { maxWidth: THUMB_SIZE * 2 + 32 }),
          ...(product.thumbnail.length === 3 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(product.thumbnail.length === 4 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(product.thumbnail.length >= 5 && { maxWidth: THUMB_SIZE * 6 }),
          ...(product.thumbnail.length > 2 && {
            position: 'relative',
            '&:before, &:after': {
              top: 0,
              zIndex: 9,
