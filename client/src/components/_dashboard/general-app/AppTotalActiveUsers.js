...
export default function AppTotalActiveUsers() {
  ...
  const runningCount = useSelector((state) => state.user.plan);
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2">Running Plan Count</Typography>
        ...
        <Typography variant="h3">{fNumber(runningCount && runningCount.runningCounter)}</Typography>
      </Box>
