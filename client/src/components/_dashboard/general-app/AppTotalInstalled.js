...
export default function AppTotalInstalled() {
  const pendingCount = useSelector((state) => state.user.plan);
  ...
  return (
    <Card sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
      <Box sx={{ flexGrow: 1 }}>
        <Typography variant="subtitle2">Pending Plan Count</Typography>
        ...
        <Typography variant="h3">{fNumber(pendingCount && pendingCount.pendingCounter)}</Typography>
      </Box>
